mod linked_list{
    use std::{rc::Rc, cell::{RefCell}, fmt::{Debug, Write}};

    //RefCell can ensure the address of data is not change, but we can modify the data inside
    type NodePtr<T> = Option<Rc<RefCell<Node<T>>>>;

    struct Node<T> where T: std::fmt::Debug + Copy{
        data: T,
        next: NodePtr<T>,
        prev: NodePtr<T>,
    }
    impl<T> Node<T> where T : std::fmt::Debug + Copy{
    ...
    impl<T> std::fmt::Debug for Node<T> where T: std::fmt::Debug + Copy {
        fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
            write!(f, "<{:?}>",self.data)
        }
    }
    impl<T> Drop for Node<T> where T: std::fmt::Debug + Copy{
        fn drop(&mut self){ 
            println!("Drop Node : {:?}", self); 
        }
    }
    pub struct LinkedList<T> where T: std::fmt::Debug + Copy{
        head: NodePtr<T>,
        tail: NodePtr<T>,
    }

    impl<T> std::fmt::Debug for LinkedList<T> where T: std::fmt::Debug + Copy {
    ...
    impl <T> LinkedList<T> where T: std::fmt::Debug + Copy{
